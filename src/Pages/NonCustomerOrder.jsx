...
const NonCustomerOrder = () => {
  ...
  const handlePhoneVerification = async () => {
    try {
      // 여기서 핸드폰 인증 요청을 보냅니다.
      const response = await axios.post("https://dev.the-goods.store/api/members/phone/auth", {
  ...
  <ButtonWrapper onClick={handleVerificationConfirm}>
    변경하기
  </ButtonWrapper>
  ...
  return (
    <Container>
      ...
      <DefaultInput
        ...
        onChange={(event) => setOrderInfo((prevInfo) => ({ ...prevInfo, receiverName: event.target.value }))} // 이 부분 수정
        placeholder="수령자명"
      />
      <DefaultInput
        type="text"
        value={orderInfo.receiverPhone} // 이 부분 수정
        onChange={(event) => setOrderInfo((prevInfo) => ({ ...prevInfo, receiverPhone: event.target.value }))} // 이 부분 수정
        placeholder="연락처"
      />
      <AddressInput
        type="text"
