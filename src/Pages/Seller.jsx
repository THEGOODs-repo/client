...
const Seller = () => {
  const [profileData, setProfileData] = useState({//초기상태
    name: "John Doe",
    bio: "잘부탁드립니다!",
    followers: 0,
    wishlist: 0,
  ...
  return (
    <div>
      <div style={sellerBoxStyle}>
        ...
        <button style={buttonStyle} onClick={handleFollowButtonClick}
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
          >{isFollowed?'팔로우 취소':'팔로우'}</button>
        <div style={profileStyle}>
          ...
          <div style={{position: 'absolute'}}>
            <button style={feedButtonStyle} onClick={handleFeedButtonClick}>피드</button>
            <button style={productButtonStyle} onClick={handleProductButtonClick}>판매 상품</button>          
          </div>          
        </div>
        <hr style={{ border: '1px solid #ddd', margin: '5vw 0' }} />
      </div>
      <div style={{marginTop:'-5.5vw'}}>
      {isFeedClicked && <PostList />}
    </div>
    <FixedButtons/>
